...
function initEventListeners() {
  ...
  btnSaveUser.addEventListener('click', () => {
    ...
    if (name) {
      ...
      if (passcode === 'aghtoyotasapporo') {
        localStorage.setItem('alice_cards_is_admin', 'true');
        localStorage.setItem('alice_cards_admin_passcode', 'aghtoyotasapporo');
      } else {
  ...
  boardViewport.addEventListener('wheel', (e) => {
    e.preventDefault();
    const zoomFactor = 1.05; // より滑らかな拡大縮小のために1.08から1.05に変更
    const rect = boardViewport.getBoundingClientRect();
    const mouseX = e.clientX - rect.left;
    const mouseY = e.clientY - rect.top;

    // 現在のマウス位置におけるボード上のローカル座標を計算
    ...
    // 新しいズーム値に基づいて、マウス位置がズレないよう panX/panY を調整
  ...
  document.getElementById('ctrl-zoom-in').addEventListener('click', () => {
    ...
    const localX = (centerX - panX) / zoom;
    const localY = (centerY - panY) / zoom;

    zoom = Math.min(zoom * 1.2, 2.0);
    panX = centerX - localX * zoom;
    panY = centerY - localY * zoom;
    updateBoardTransform();
  });
...
// 掲示板タッチ操作用の変数
let touchMode = 'none'; // 'pan' または 'pinch'
...
async function fetchCards() {
  try {
    ...
    showFetchErrorOnBoard(err.message);
  }
}

// データベース取得エラー時のUI表示
function showFetchErrorOnBoard(message) {
  cardsLayer.innerHTML = '';
  listCardsLayer.innerHTML = '';
  
  const errorMsg = document.createElement('div');
  errorMsg.className = 'board-empty-message';
  errorMsg.style.color = '#ef4444';
  errorMsg.innerHTML = `
    <p style="font-weight: bold; font-size: 18px; color: #ef4444; margin-bottom: 8px;">カードデータの取得に失敗しました</p>
    <p style="font-size: 13px; color: var(--text-muted); margin-bottom: 16px;">${escapeHtml(message)}</p>
    <button onclick="fetchCards()" class="btn-icon-text primary-btn" style="padding: 8px 16px; font-size: 13px; border-radius: 8px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; background: var(--primary); color: white;">
      <span>再試行する</span>
    </button>
  `;
  ...
  if (currentView === 'board') {
    ...
    errorMsg.style.left = '1200px';
    errorMsg.style.top = '850px';
    errorMsg.style.textAlign = 'center';
    cardsLayer.appendChild(errorMsg);
  } else {
    errorMsg.style.textAlign = 'center';
    errorMsg.style.margin = '100px auto';
    listCardsLayer.appendChild(errorMsg);
  }
}
